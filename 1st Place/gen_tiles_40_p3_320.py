...
parser.add_argument(
    "--raw_img_dir", help="Directory containing pyramidal whole slide tif files", required=True
)
parser.add_argument(
...
slide.BASE_DIR = f"workspace/data/{STAGE}"

meta = pd.read_csv(META_CSV_PATH)

MAX_SCORE_THRESH = 0.5
MIN_TILES_PER_PAGE = 32
MAX_TILES_PER_PAGE = 512
SCORE_THRESHS = [i / 10 for i in range(int(MAX_SCORE_THRESH * 10), -1, -1)]


PAGE_IX_MULS = {i: 2 ** (BASE_PAGE - i) for i in range(BASE_PAGE + 1)}
DIR_OUTPUT_TILES = f"./workspace/data/{STAGE}/tiles"


PAGES_TO_EXTRACT = {}
PAGES_TO_EXTRACT[40] = [2]
...
def save_tiles_for_page(cur_page, base_sz, slide_name, df_tiles):
    ...
    for idx, row in df_tiles.iterrows():
        ...
        try:
            ...
            img = np.ndarray(
                buffer=region, dtype=np.uint8, shape=(region_height, region_width, 3)
            )  # rgb image
...
def generate_tiles_for_slide_list(slide_names, base_sz, pages_to_extract):
    for slide_name in slide_names:
        # ##generate tiles
        df = pd.read_csv(
            f"{slide.TILE_DATA_DIR}/{slide_name}-tile_data.csv", skiprows=14
...
PATCH_SIZES_ACT = {
    i: BASE_SZ * 2 ** (BASE_PAGE - i) for i in range(BASE_PAGE)
}  # patch size to extract for each page

print(f"##### GENERATING TILE META {BASE_SZ} tile sizes: {PATCH_SIZES_ACT} #####")
...
print(f"##### GENERATING TILES {BASE_SZ}_{pg}_{sz} #####")
multiprocess_generate_tiles(NAMES, BASE_SZ, pages_to_extract=PAGES_TO_EXTRACT[BASE_SZ])

elapsed = time.time() - START_TIME
print(
    f"##### DONE GENERATING TILES {BASE_SZ}_{pg}_{sz} TOTAL TIME: {timedelta(seconds=elapsed)} #####"
)
gc.collect()
