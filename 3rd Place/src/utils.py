...
def predict_proba(X, C, I):
    """
    predicts the probas of samples X using one or more logregs
    """
    z = (X @ C.T) + I
    proba = 1 / (1 + np.exp(-z))
    return proba
...
def make_auto_mask(slide, mask_level):
    ...
    mask = opening(
        closing(np.logical_and(im_gray < t, im_gray > 0.02).reshape(size), footprint=square(2)),
        footprint=square(2),
    )
    final_mask = mask
    return mask
...
def grid_blob(slide, point_start, point_end, patch_size, analyse_level):
    """
...
def check_patch(
    slide, mask, coord_grid_0, mask_level, patch_size, analyse_level, mask_tolerance=0.5, margin=0
):
    """
    ...
    slide_png = np.array(
        slide.read_region(
            (0, 0),
            mask_level,
            slide.level_dimensions[mask_level],
        )
    )[:, :, :3]
    assert (
        slide_png.shape[0:2] == mask.shape[0:2]
    ), "Raise value, mask not of the right shape {}".format(mask.shape[0:2])
        ...
        if mask_percentage(mask, point_cent_l, radius, mask_tolerance):
            ...
            if still_add:
                sub_param = [
                    coord_0[1] - margin,
                    coord_0[0] - margin,
                    patch_size[0] + 2 * margin,
                    patch_size[1] + 2 * margin,
                    analyse_level,
                ]
...
def patch_sampling(
    ...
    grid_coord = grid_blob(slide, point_start_0, point_end_0, patch_size, analyse_level)

    margin_mask_level = get_size(slide, (overlapping, 0), 0, analyse_level)[0]
    parameter = check_patch(
        slide,
        wsi_mask,
        grid_coord,
        mask_level,
...
def prepare_clini_data(clini, train: bool = True):
    """
    Prepare the clinical data for the model.
    params:
        clini: pd.DataFrame, clinical data
        train: bool, if True, fit the StandardScaler, if False, load the StandardScaler
    return:
        X: np.array, clinical data
        filename_to_index: dict, mapping from filename to index in X
    """
    filename_to_index = {f: o for o, f in enumerate(clini["filename"].values)}
    ...
    if train:
        ...
        stdc = joblib.load("./models/standardscale_clini.joblib")
        X = stdc.transform(X)
    return X, filename_to_index
